...
def get_image_feature_vectors():
    ...
    for filename in glob.glob('temp_images/*.jpg'):  # assuming gif
        i = i + 1
...
def download_image(image_url,file_identifier):
    file_path = "temp_images/"+file_identifier+".jpg"
...
def process_video(video_url_path):
    with youtube_dl.YoutubeDL(ydl_opts) as ydl:
        video = ydl.extract_info(video_url_path,download=False)

        channel_name = video['uploader']
        channel_name = channel_name.replace(" ","")

        video_thumbnail_url = video['thumbnails'][-2]['url'].split('?')[0]
        video_id = video['id']

        file_identifier = channel_name + str(video_id)
        if not os.path.exists("temp_images/"):
            os.makedirs("temp_images/")

        download_image(video_thumbnail_url,file_identifier)


def compare_two_video_thumbnails(video_link_first,video_link_second):
    process_video(video_link_first)
    process_video(video_link_second)
...
print(compare_two_video_thumbnails("https://www.youtube.com/watch?v=LyC855KdBKo",
